<?php
// start the session
 session_start();
// disable notification & warning
 error_reporting(E_ALL & ~E_NOTICE && ~E_WARNING); 
 // if the user not logged in
if(!$_SESSION['loggedInUser']){

  //send them to the login page
  header("Location: ../../../index.php");
}
########################################
# get the ID send by the GET collection 
########################################
$vacancyID = $_GET['id'];

// connect to database
include ("../../../includes/controls/db/connection.php");
// include functions.php
include ("../../../includes/controls/controls/functions.php");

//query the data using the reseved clientID
$query = "SELECT * FROM  vacancy WHERE id='$vacancyID'";
$result = mysqli_query($conn, $query);

// if the result is re4turned
if(mysqli_num_rows($result) > 0){
    while ($row = mysqli_fetch_assoc($result)) {
        $back_date    = $row['back_date'];
        $vacancy_id    = $row['vacancy_number'];
    }
    
}else{
    // no result returned
    $alertMessage = "<div class='alert alert-success'>لا توجد نتيجة<a href='' data-dismiss='alert' class='close pull-left'>&times;</a></div>";
}

// if the update buttom is pressed
if( isset( $_POST['update'] ) ){

    $back_date = validateFormData($_POST['back_date']); 
    $vacancy_number   = validateFormData($_POST['vacancy_number']);
    
       // new database query & result
       $query = "UPDATE vacancy SET vacancy_number = '$vacancy_number',"
       ."back_date = '$back_date' WHERE id = $vacancyID";
       
    if(mysqli_query($conn,$query)){
        header("Location: vacancy.php");
    }else{
        $usedBefore = "<div class='alert alert-danger a' style='font-size: 18px'>رقم الميدانية مستخدم مسبقا *<a class='close' data-dismiss='alert'>&times;</a></div>";
   
    }
    //header("Location: soldiers.php");
}
	
// if the update buttom is pressed
if( isset($_POST['delete'] ) ){
$alertMessage = "<div class='alert alert-danger'>
        <h5>هل أنت متأكد من حذف العنصر ...</h5>
        <form action='".htmlspecialchars($_SERVER["PHP_SELF"])."?id=$vacancyID' method='post'>
            <input type='submit' class='btn btn-danger' name='confirm-delete' value='مسح '>
            <a type='button' class='btn btn-primary' data-dismiss='alert' name='no-thanks'>لا شكرا</a>
        </form>
    </div>";

    
}
if(isset( $_POST['confirm-delete'] )){
           // new database query & result
            $query = "DELETE FROM vacancy WHERE id='$vacancyID'";
            $result = mysqli_query($conn,$query);
            if($result){
                header("Location: vacancy.php");
            }else{
                echo "<div class='alert alert-danger'>خطــــــأ </div>";
            }
          //header("Location: clients.php");
      }
if(isset( $_POST['no-thanks'] )){
	unset($_POST['delete']);
}
$query = "SELECT * FROM  vacancy WHERE id='$vacancyID'";
$result = mysqli_query($conn, $query);


 // close the connection

 // include the header
 include ("../../../includes/header/header/header.php");
 
    ?>
<div class="container">

        <h2 class="page-header">
		تعديل ميدانية
		</h2>
		<?php echo $alertMessage; ?>
		<?php
			echo $usedBefore;
			echo $requiredError;
			echo $invalidEmail;
			
		?>
        <form action="<?php htmlspecialchars($_SERVER["PHP_SELF"])?>" method="post" class="form-horizontal">
           
			  <div class="form-group">
				<div class="col-sm-2"></div>
			    <div class="col-sm-8">
				  <input type="text" class="form-control1" id="vacancy_number" name="vacancy_number" value="<?php echo $vacancy_id?>">
				</div>
				<label for="vacancy_id" class="col-sm-1 control-label">الأسم</label>
				</div>
				<div class="form-group">
				<div class="col-sm-2"></div>
				<div class="col-sm-8">
				  <input type="date" class="form-control1" id="back_date" name="back_date" value="<?php echo $back_date?>">
				</div>
				<label for="back_date" class="col-sm-1 control-label">تاريخ النوبتجية</label>
			  </div>
				
			   <div class="form-group">        
				  <div class="col-sm-10">
                    <a href="vacancy.php" type="button" class="btn btn-default">إلغاء</a>
                    <button type="submit" class="btn btn-success " name="update">تعديل</button>
					<button type="submit" class="btn btn-danger" name="delete" >حذف</button>
                </div>
				</div>
            <div class="col-md-push-1 col-md-9">
			</div>
            </div>
        </form>
     </div>
	  <?php include "../../../includes/footer/footer/footer.php";?>
  </body>
</html>