...
if( isset( $_POST["add"])){
   ...
   if( $soldier_id && $type){
            ...
            $query = "INSERT INTO  soldiersjobs (soldier_id,type)
			VALUES ('$soldier_id','$type')";
...
<div class="container">
		...
		?>
        <h2 class="page-header">إنشاء وظيفة جندى [<?php echo $sarya?>]</h2>
				<?php
					echo $usedBefore;
					echo $requiredError;
					echo $invalidEmail;
        ...
        <form action="<?php htmlspecialchars($_SERVER["PHP_SELF"])?>" method="post" class="form-horizontal">
			  ...
			  <div class="form-group">
			    ...
			    <select class="form-control" name="type" id="type">
			      	...
			      	<option value="1">بوليس</option>
			      	<option value="2">خط</option>
			      	<option value="3">كانتين</option>
			      	<option value="4">مندوب</option>
			      	<option value="5">منوب</option>
			      	<option value="6">أمن</option>
			      	<option value="7">مطبخ</option>
			      </select>  
					</div>
			   ...
			   <div class="form-group">        
				  <div class="col-sm-10">
					...
					<button type="submit" class="btn btn-success" name="add">إضافة وظيفة </button>
				  </div>
				</div>
