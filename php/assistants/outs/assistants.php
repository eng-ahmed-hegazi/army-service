...
// query & مأموريات
$query1 = "SELECT assistantout.id,assistant.name,assistant.degree,assistantout.destination,assistantout.start_date,assistantout.end_date,assistantout.type FROM assistantout  JOIN assistant ON  assistant.id=assistant_id ORDER BY assistant.degree";
$result1 = mysqli_query( $conn, $query1 );
// query & فرق
$query2 = "SELECT assistantout.id,assistant.name,assistant.degree,assistantout.destination,assistantout.start_date,assistantout.end_date,assistantout.type FROM assistantout  JOIN assistant ON  assistant.id=assistant_id ORDER BY assistant.degree";
$result2 = mysqli_query( $conn, $query2 );
// query & غياب
$query3 = "SELECT assistantout.id,assistant.name,assistant.degree,assistantout.destination,assistantout.start_date,assistantout.end_date,assistantout.type FROM assistantout  JOIN assistant ON  assistant.id=assistant_id ORDER BY assistant.degree";
$result3 = mysqli_query( $conn, $query3 );
// query & مستشفى
$query4 = "SELECT assistantout.id,assistant.name,assistant.degree,assistantout.destination,assistantout.start_date,assistantout.end_date,assistantout.type FROM assistantout  JOIN assistant ON  assistant.id=assistant_id ORDER BY assistant.degree";
$result4 = mysqli_query( $conn, $query4 );

$d=strtotime("+1 day");
$today = date("Y-m-d", $d);
$query10 = "SELECT * FROM  assistantout";
		...
		while ($row = mysqli_fetch_assoc($result10)) {
			...
			$query = "DELETE FROM assistantout WHERE id='$id'";
			$result = mysqli_query($conn,$query);
