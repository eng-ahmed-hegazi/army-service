...
$query = "SELECT * FROM  assistantpunishment WHERE id='$assistantID'";
...
if( isset( $_POST['update'] ) ){
	...
	$date   = validateFormData($_POST['date']);
       ...
       $query = "UPDATE assistantpunishment SET assistant_id = '$assistant_id',"
       ."officer_id = '$officer_id',order_number='$order_number',crime_content='$crime_content',punishment='$punishment',
	   crime='$crime',period='$period',date='$date' WHERE id = $assistantID";
...
if(isset( $_POST['confirm-delete'] )){
			/*$query1 = "SELECT * FROM assistantpunishment WHERE id='$assistantID' LIMIT 1";
			...
			gal
			$sql = "UPDATE assistant SET availability=1 WHERE id='$assistantId'";
			$update = mysqli_query($conn, $sql);
			*/
           // new database query & result
            $query = "DELETE FROM assistantpunishment WHERE id='$assistantID'";
            $result = mysqli_query($conn,$query);
...
<div class="container">
		...
		تعديل عقوبة إنضباطية للصف ضابط 
		/
		<?php
		$query1 = "SELECT * FROM  assistant WHERE id=$assistant_id LIMIT 1";
		$result1 = mysqli_query($conn , $query1);
        ...
        <form action="<?php htmlspecialchars($_SERVER["PHP_SELF"])?>" method="post" class="form-horizontal">
			  <div class="form-group">
			    ...
			    <div class="col-sm-8">
					<select class="form-control" name="assistant_id" id="assistant_id">
						...
						<?php
						$query = "SELECT * FROM  assistant WHERE sarya=$sarya ORDER BY degree";
						$result = mysqli_query($conn , $query);
						if( mysqli_num_rows($result) > 0){
						$i=0;
						while ($row = mysqli_fetch_assoc($result)) {
							
							if($assistant_id==$row['id']){
			  ...
			  <div class="form-group">
			    ...
			    <select class="form-control" name="punishment" id="punishment">
			      	...
			      	<option value="1" <?php if ($punishment==1) echo "selected" ?>>قطع من الراتب</option>
