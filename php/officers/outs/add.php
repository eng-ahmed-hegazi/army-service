...
if( isset( $_POST["add"])){
   ...
   if(!$_POST['destination']){
       $destinationError = "من فضلك حدد تاريخ النوبتجية<br>";
   }else {
   ...
   if( $officer_id && $type && $destination && $start_date && $end_date){

            $query = "INSERT INTO  officerout (officer_id,type,destination,start_date,end_date)
	VALUES ('$officer_id','$type','$destination','$start_date','$end_date')";
	
            $result = mysqli_query($conn, $query); 
			$sql = "UPDATE officer SET availability=0 WHERE id='$officer_id'";
 ...
 include ("../../../includes/header/header/header.php");
    ...
    ?>
<div class="container">
        <h2 class="page-header">قيام مأمورية / فرقة ضباط</h2>
		<?php
			echo $usedBefore;
			echo $requiredError;
			echo $invalidEmail;
        ...
        <form action="<?php htmlspecialchars($_SERVER["PHP_SELF"])?>" method="post" class="form-horizontal">
			  ...
			  <div class="form-group">
			    ...
			    <select class="form-control" name="type" id="type">
			      	...
			      	<option value="1">مأمورية</option>
			      	<option value="2">فرقة</option>
			      	<option value="3">أختبارات</option>
			      	<option value="4">مستشفى</option>
			      </select>  
					</div>
			  ...
			  <div class="form-group">
				<div class="col-sm-2"></div>
					<div class="col-sm-8">
					  <input type="text" class="form-control1" id="destination" name="destination" >
					</div>
			  ...
			  <div class="form-group">
			   ...
			   <div class="form-group">        
				  <div class="col-sm-10">
					...
					<button type="submit" class="btn btn-success" name="add">إضافة مأمورية / فرقة </button>
				  </div>
				</div>
