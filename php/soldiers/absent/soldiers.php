...
// query & مأموريات
$query1 = "SELECT soldierabsent.id,soldier.sarya,soldier.name,soldier.degree,soldierabsent.start_date,soldierabsent.end_date,soldierabsent.type FROM soldierabsent  JOIN soldier ON  soldier.id=soldier_id ORDER BY soldier.degree";
$result1 = mysqli_query( $conn, $query1 );

$d=strtotime("+1 day");
$today = date("Y-m-d", $d);
$query10 = "SELECT * FROM  soldierabsent";
		...
		while ($row = mysqli_fetch_assoc($result10)) {
			...
			$query = "DELETE FROM soldierabsent WHERE id='$id'";
			$result = mysqli_query($conn,$query);
...
<?php if(count($result1)!=0) {?>
  ...
  <table class="table  table-bordered text-center">
      ...
      // output the data
	   while ($row = mysqli_fetch_assoc($result1)) {
		...
		switch($row['type']){
			case 1:
			$type = '<label>غياب </label>';
			break;
			case 2:
			$type = '<label>سجن حربى</label>';
			break;
		}
  ...
  <tr>
      <td colspan="8">
          <div class="text-center"><a href="absent.php" type="button" class="btn btn-sm btn-success"> أضافة غياب /سجن</a>
