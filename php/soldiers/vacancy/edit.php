...
$query = "SELECT * FROM  soldiervacancy WHERE id='$soldierID'";
...
if( isset( $_POST['update'] ) ){
    ...
    $end_date   = validateFormData($_POST['end_date']);
       ...
       $query = "UPDATE soldiervacancy SET soldier_id = '$soldier_id',type='$type',start_date='$start_date',end_date='$end_date' WHERE id = $soldierID";
...
if(isset( $_POST['confirm-delete'] )){
			$query1 = "SELECT * FROM soldiervacancy WHERE id='$soldierID' LIMIT 1";
           ...
           // new database query & result
            $query = "DELETE FROM soldiervacancy WHERE id='$soldierID'";
            $result = mysqli_query($conn,$query);
        ...
        <form action="<?php htmlspecialchars($_SERVER["PHP_SELF"])?>" method="post" class="form-horizontal">
			  ...
			  <div class="form-group">
			    ...
			    <select class="form-control" name="type" id="type">
			      	...
			      	<option value="4" <?php if ($type==5) echo "selected" ?>>عارضة</option>
			      </select>  
					</div>
