...
// query & مأموريات
$query1 = "SELECT soldiervacancy.id,soldier.sarya,soldier.name,soldier.degree,soldiervacancy.start_date,soldiervacancy.end_date,soldiervacancy.type FROM soldiervacancy  JOIN soldier ON  soldier.id=soldier_id ORDER BY soldier.degree";
$result1 = mysqli_query( $conn, $query1 );

$d=strtotime("+1 day");
$today = date("Y-m-d", $d);
$query10 = "SELECT * FROM  soldiervacancy";
		...
		while ($row = mysqli_fetch_assoc($result10)) {
			...
			$query = "DELETE FROM soldiervacancy WHERE id='$id'";
			$result = mysqli_query($conn,$query);
...
<?php if(count($result1)!=0) {?>
  ...
  <tr>
      <td colspan="8">
          <div class="text-center"><a href="vacancy.php" type="button" class="btn btn-sm btn-success"> أضافة أجازة</a>
